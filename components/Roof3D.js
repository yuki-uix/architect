...
export default function Roof3D({ data, onLayerSelect, selectedLayer }) {
  ...
  const toggleExplosion = (explode) => {
    ...
    layerObjectsRef.current.forEach((mesh, index) => {
      ...
      for(let i = 0; i < index; i++) {
        accumulatedHeight += data.layers[i].height;
      }
      const resetY = data.initialY + accumulatedHeight + layer.height / 2;
      const finalY = explode ? currentY + layer.height / 2 + index * data.explodedYGap : resetY;
